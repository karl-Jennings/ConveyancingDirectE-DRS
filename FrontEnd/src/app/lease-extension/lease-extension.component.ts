...
@Component({
  selector: 'app-lease-extension',
  templateUrl: './lease-extension.component.html',
  styleUrls: ['./lease-extension.component.css']
})
export class LeaseExtensionComponent implements OnInit {
  ...
  PushTitleToGrid() {
    ...
    if (this.titleGroup.valid) {
      ...
      if (this.titleList.find(s => s.LocalId == this.selectedTitleNumber) == null) {
        ...
        insertObj.LocalId = this.selectedTitleNumber;
        this.titleList.push(Object.assign({}, insertObj));
  ...
  SelectTitleRow(id: any) {
    ...
    var selectedObj: any = this.titleList?.find(s => s.LocalId == id);
    this.selectedTitleNumber = selectedObj.LocalId;
    this.titleGroup.setValue(selectedObj);
  }
